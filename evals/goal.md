...
- 同日对 `platform-file:a9a1d4a` 做独立安全留出：模型返回 clean，但人工复核确认单删和批量删除都移除了 `fileStorageService.delete(...)`，导致 OSS/本地对象残留。新增对象生命周期确定性预检和 `missed` 人工标签状态后，该样本 scorecard 为 1 个 confirmed P1、模型命中 0 个，召回率 0%；这次漏报已纳入后续回归，不能再被 clean 输出隐藏。
