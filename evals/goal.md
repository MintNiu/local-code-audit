...
- 阶段 0：已通过。默认门禁完成 5/5 正例、5/5 负例；除法、URL 凭证泄漏和租户隔离三类正例共 4 个已知根因每次全部命中，三个 clean 负例没有 P0～P3，输出哈希分别保持一致，截断故障路径显式失败。无模型 `test-preflight.sh` 另外覆盖构建完整性、跨 hunk SSRF、URL builder、路径 API 别名、多处 Java 除法和 guard 边界；`run-synthetic.sh` 会先执行该门禁。
