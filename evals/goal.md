...
运行器新增受限的跨事务/行锁文本证据：变更 Java 文件命中 `@Transactional`、`find*ForUpdate` 或 `FOR UPDATE` 时，把变更文件及相同锁接收者的关联 Java 文件摘要放入 prompt。它只提供文本上下文，不自动生成 finding，也不等于检测到死锁；证据有上限，预算不足时可降级跳过。无模型分片回归已覆盖变更文件、未变更关联文件和行锁调用。
