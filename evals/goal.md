...
- 构建完整性预检：审查器会从新增/修改的 Java import 中提取同仓库前缀类型，并把当前源码索引中找不到的候选证据注入模型；显式 `--context` 文件仍引用当前提交删除的 Java 类型时，也会生成跨仓库 P1 候选。分片只接收自身文件的预检证据。外部依赖、生成源码、通配符 import 和无法确认的候选仍必须结合项目构建上下文人工确认，不能把预检单独当作编译结果。
