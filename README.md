...
When the collected diff exceeds `OLLAMA_REVIEW_MAX_DIFF_BYTES` (default `3000`), `local-review` automatically performs deterministic file- and unified-hunk-boundary sharding. Each shard is reviewed separately; only byte-identical repeated paragraphs are removed during aggregation, while distinct findings remain visible. The general command uses `OLLAMA_REVIEW_CHUNK_TIMEOUT_SECONDS=180` and `OLLAMA_REVIEW_CHUNK_NUM_PREDICT=2048`; the personal high-performance wrapper keeps the same 3000-byte shard budget but raises the shard output budget to 4096. Any shard timeout, truncation, or invalid output fails the whole review and prints completed shards only as diagnostic output.
...
Transient Ollama transport failures are retried up to two times within the total review timeout by default. Set `OLLAMA_REVIEW_RETRY_ATTEMPTS=0` to disable retries or tune it for local stability. The launcher also estimates the input budget for system rules, context, and diff before sending; tune `OLLAMA_REVIEW_INPUT_RESERVE_TOKENS` when you need a different safety margin.
...
After changing rules or parameters, run `./scripts/verify-runtime.sh` to read-only check that the installed Ollama tuned model still matches the current `config/Modelfile`. If it fails, follow the printed rebuild commands; the script never recreates models automatically.
