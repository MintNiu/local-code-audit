...
在此基础上，脚本增加了窄范围的确定性反向锁序预检：它把带 `@Transactional` 的源码段中的直接 `receiver.*ForUpdate(...)` 按文本顺序配对，只有跨源码段/文件出现反向顺序且至少一侧是变更文件时才产生 P1 候选。输出会明确说明“候选”和需要确认的资源映射、调用链与并发条件；候选在模型输出之后合并，不放进 prompt，避免模型重复和长上下文截断。这不是 RAG，也不是完整死锁证明，更不是 Java 调用图分析；间接调用和运行时 SQL 仍要靠人工审计或并发集成测试确认。

ERP 留出最初因私有清单 parent SHA 错误而把历史差异误当当前提交，相关运行全部作废。修正 parent 后，`736ef17845a9a45e55a262d6d64ea81738773059` 是单文件 77/58 变更，7 个分片、51 秒、exit=0，但模型返回 clean；只读人工审计确认销售订单→退货单与退货单→销售订单的反向锁序可形成 P1 死锁候选，因此它记录为真实漏报，不计入“通过”或模型召回统计。旧错误 parent 的超时/分片结果不进入任何 scorecard。

后续 32768 context 的 ERP 诊断出现分片超时，较小上下文诊断也出现模型超时/截断；运行器按 fail-closed 处理，未把不完整结果计入 scorecard。确定性预检虽然能在失败诊断中保留反向锁序候选，但这只证明候选被观测到，不等于完整模型审查成功。

随后用冻结后的个人审查器和同一正确 parent 复核 `7fee9f1a2af94fdc6d47769a7e93f2d7a493f140`：`num_ctx=32768`、12 个分片、151 秒、exit=0、结果 clean。人工确认退款草稿的退货单→收货单→退款单与退款确认的退款单→退货单顺序，在“已有退款草稿且并发确认/创建”前提下形成 P1 死锁候选；模型未报出该候选，故同样只作为真实漏报证据保留，不进入“通过”统计。两条正确 parent 的结果都没有写入公开 scorecard，待人工真值清单扩展后再统计。

评测链路现在还要求 `output_complete=true` 和标签中的 `# verdict` 与 finding 行一致，并提供 `./evals/stage1-gate.sh` 检查真实评测集的提交数、train/dev/holdout 切分、功能簇隔离、P0/P1 召回、定位准确率、误报率和重复稳定性。当前真实标签仍是私有数据，公开夹具只证明门禁逻辑；在 holdout 分母达到要求前，不宣称生产级高可用。
...
### 2026-09-24：补充跨服务超时与调度关闭 clean 留出
...
随后在系统规则中加入了统一 diff 的安全修复语义，并短暂尝试在私有 few-shot 中加入不含真实密钥的“删除旧字面量、改用环境变量”负例。重建 tuned 模型后，模型仍有时输出带 `P1` 严重级别但正文承认“旧凭据已移除、无需修复”的自相矛盾段落；同时长 few-shot 会增加部分租户样例的响应超时，因此该示例已撤回，不作为默认输入。运行器增加了一个证据门：仅当配置差异的所有当前/上下文凭据字段都已变成 `${ENV_NAME}` 占位符、没有当前字面量值、且该段明确表示无需修复时，才丢弃这一自相矛盾段；含有“仍在/另外”等独立问题线索的段落不会被丢弃。该门不按关键词隐藏结构化候选，真实 `platform-file:7eb92a1` 字面量凭据正例仍保留 P1；配置输出回归从 9 个扩展到 11 个用例并通过，其中还验证了同一配置保留另一份字面量时 finding 不会被隐藏。复跑 `2e33eaf` 后结果为唯一 clean（exit=0，25 秒），因此本次人工复核结论为 `gold_p0_p1=0`、`predicted_candidates=0`、`false_positive=0`；对应新结果与标签仍保存在本机私有评测目录，没有覆盖先前 3 条候选的旧 scorecard。该修复仍需更多安全修复反例和跨格式配置验证，不能据此宣称通用安全召回率已达生产标准。

### 2026-09-25：迁移大提交复测补齐第二个真实 P1

针对前一轮 `platform-hr-service:a8bf560` 的大提交，又在同一父提交快照上使用 `num_ctx=32768`、9KB 分片完整重跑。28 个分片全部成功，耗时 554 秒，结果没有截断；个人 tuned profile 保留了字面量内部/OSS 令牌 P1，并同时识别出 `sql/hr_assignment_history_integrity_upgrade.sql:33` 的触发器 DROP/CREATE 名称不一致 P1。该迁移在重放时可能留下旧触发器，并在创建阶段因名称冲突或引用不一致失败。

本次人工标签将 12 个候选拆为 2 条 confirmed、8 条有直接代码证据的 false-positive、2 条 uncertain（一个是多文件删除占位符聚合、一个是跨文件生成列提示，当前模型定位格式不足以安全逐条映射）。复测 scorecard 为 `gold_p0_p1=2`、`p0_p1_found=2`、`predicted_candidates=12`、`false_positive=8`、完整运行 `true`，P0/P1 召回为 100%。这只说明该具体大提交的两个已确认根因均被发现，不能外推为通用生产召回率；8 分钟以上耗时也不适合作为日常默认参数。前一轮 15KB 分片超时记录和 19 候选的初始 scorecard 保留为历史失败对照，不覆盖。
