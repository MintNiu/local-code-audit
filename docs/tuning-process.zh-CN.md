...
分片不是 RAG：没有向量库、Embedding 或相似度召回，项目规则和显式上下文仍由提示词传入。分片请求默认去掉私有 few-shot 示例以降低重复上下文和耗时，但仍保留系统审计规则、项目规则、README（如果显式开启）和 Git 状态。可以通过 `OLLAMA_REVIEW_CHUNK_TIMEOUT_SECONDS`、`OLLAMA_REVIEW_CHUNK_NUM_PREDICT` 和 `OLLAMA_REVIEW_MAX_DIFF_BYTES` 调整。
...
在当前规则和路径/格式门禁下，私有历史试跑中 `91bff253` 完整返回耗时 68 秒；重建 tuned 模型后，`2f6c3934` 完整返回耗时 146 秒；后者仍产生多条需要人工判定的接口契约和测试泛化建议。这证明分片解决了完整性/超时的一部分工程问题，但没有把模型自动变成低误报审计器，因此阶段 1 仍必须逐条人工标注。
