...
真实提交试跑还暴露了一个与 RAG 无关的工程问题：一个只有约 6 KB、但包含多个文件的提交仍可能让单次生成达到长度上限。于是 `local-review` 增加了按 `diff --git` 文件边界分片的回退链路：差异超过 6000 字节时，每个分片单独请求模型，结果按分片顺序拼接，不做去重或摘要；任何一个分片超时、截断或缺少严重级别/位置，整次审查都失败，并把已经完成的分片仅以诊断形式展示。
